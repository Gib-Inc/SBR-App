...
| **Contribution margin** | `contribution-margin-service.ts` (`getBlendedContributionMargin`; COGS + channel fees + MEASURED freight ~11.8% of revenue, Amazon-FBA gap disclosed) | `revenue × 0.6` or any hardcoded margin; COGS fallback is `COGS_PLUG_RATE` (35%, QB-validated) never 40%; margins that silently exclude freight |
| **Runway margin rate** | `runwayMarginRate` from the contribution engine ((rev − COGS − Amazon referral − freight)/rev; freight is a COGS-type QB account, NOT inside the OpEx overhead the runway subtracts) | gross margin, net margin, or the fee-inclusive contribution rate (Shopify fees already live in QB overhead — fee PLACEMENT differs by channel) |
| **Marketing efficiency (the hero number)** | `marketing-truth-service.ts` (`getMarketingTruth` — composes governor MER + contribution margin + feed-confidence; NO math of its own) | per-channel last-click pixel ROAS as a decision number; any MER rendered without the feed-confidence layer |
