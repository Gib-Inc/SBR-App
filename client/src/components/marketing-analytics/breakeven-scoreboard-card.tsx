...
interface MerMonth { month: string; revenue: number; marketing: number; mer: number | null; vsBreakeven: number | null; belowBreakeven: boolean; netIncome: number | null; }
interface Resp {
  success: boolean; breakeven: number; monthly: MerMonth[]; monthsBelowBreakeven: number;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border bg-muted/30 p-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <span className="rounded bg-background border px-2 py-1"><span className="text-muted-foreground">Platform-tracked media</span> <span className="font-semibold">{k(c.mediaSpend)}</span></span>
            <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
            <span className="rounded bg-amber-50 border border-amber-200 px-2 py-1 dark:bg-amber-950/30 dark:border-amber-900/40"><span className="text-muted-foreground">+ agency / untracked</span> <span className="font-semibold text-amber-700 dark:text-amber-300">{c.agencyGap != null ? k(c.agencyGap) : "—"}</span></span>
            <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
          ...
          <p className="text-[11px] text-muted-foreground mt-1.5">
            The app's tracked media ({k(c.mediaSpend)}) is only part of what you spend — QuickBooks books {c.qbRunRate != null ? k(c.qbRunRate) : "more"} once the Carpe Diem / CDUK agency fee and untracked spend are in. MER uses the full cost; that's why it's lower than the media-only ROAS.
            {c.qbPending && " Current month's QB isn't closed yet, so MER uses your last verified month as the run-rate (the real figure will firm up when the month books)."}
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {data.monthly.map((m) => (
              <div key={m.month} className="flex items-center gap-2 text-sm" data-testid={`mer-month-${m.month}`}>
                <span className="w-14 text-xs text-muted-foreground">{fmtMonth(m.month)}</span>
