...
*   **AI Agent Settings**: Per-user automation settings stored in `ai_agent_settings` table. Includes: autoSendCriticalPos (auto-send POs for critical stock), criticalRescueDays (stockout threshold for auto-send), shopifyTwoWaySync (push inventory to Shopify), shopifySafetyBuffer (stock reserve for Shopify sync), amazonTwoWaySync (push inventory to Amazon), amazonSafetyBuffer (stock reserve for Amazon sync). Managed via `/api/ai-agent-settings` endpoint and configured in AI Agent → Rules tab.
...
*   **Amazon Two-Way Sync**: AmazonClient (server/services/amazon-client.ts) supports region-based SP-API endpoints (NA/EU/FE), listing fetch via Catalog Items API v2022-04-01, and inventory push via Listings API. AmazonInventorySyncService (server/services/amazon-inventory-sync-service.ts) respects amazonTwoWaySync toggle and logs dry-run events when disabled. AI Agent Rules includes amazonTwoWaySync toggle and amazonSafetyBuffer setting. Integration Settings shows region selector (NA/EU/FE), push inventory toggle, and sync mode display (1-Way/2-Way). API endpoints: GET /api/integrations/amazon/products (fetch listings for SKU mapping), POST /api/amazon/sync-inventory (push inventory), GET /api/amazon/sync-status. Amazon SellerSKU maps to items.amazonSku field for cross-channel product matching.
*   **Read-Only Integrations**: Extensiv is strictly read-only for inventory. EXTENSIV_SYNC updates pivotQty from Extensiv snapshots but never writes back. Returns go to HILDALE only (not Extensiv). Amazon is read-only by default but supports two-way sync when amazonTwoWaySync is enabled in AI Agent Rules. Shopify orders are imported but inventory is owned by this app.
...
*   **Enhanced SKU Mapping Wizard**: Products → SKU Mapping Wizard button opens multi-channel mapping interface. Shopify tab fetches products via `/api/integrations/shopify/products` endpoint and provides auto-matching by UPC (highest priority) or SKU. Auto-suggested matches show with one-click confirmation. Manual variant selection available via searchable dropdown. Successful links save shopifyProductId, shopifyVariantId, shopifyInventoryItemId, and shopifySku to the item. Uses precomputed Maps for O(1) lookup performance on large catalogs. "Apply All Suggested Matches" button bulk-applies all auto-detected mappings.
*   **Shopify Webhook Integration**: POST /api/webhooks/shopify receives real-time order events (orders/create, orders/updated, orders/cancelled) from Shopify. HMAC signature verification via SHOPIFY_WEBHOOK_SECRET. Auto-creates SalesOrders, maps SKUs to internal products (shopifySku or sku), adjusts inventory via InventoryMovement, and logs SKU_MISMATCH for unmapped items.
