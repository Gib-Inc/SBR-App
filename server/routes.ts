...
export async function registerRoutes(app: Express): Promise<Server> {
   ...
   * Creates draft PO opportunities in GoHighLevel for high/critical priority items
   * 
    ...
    try {
      ...
      if (!apiKey || !locationId) {
        return res.status(400).json({ 
          error: "GoHighLevel not configured. Please set up GHL integration in Data Sources.",
          primaryGhlOpportunityUrl: null 
        });
      }
      
      if (!pipelineId || !stageDraftId) {
        return res.status(400).json({ 
          error: "GHL Pipeline not configured. Please set GHL_PURCHASE_PIPELINE_ID and GHL_PURCHASE_STAGE_DRAFT_ID.",
          primaryGhlOpportunityUrl: null 
        });
      }
      ...
      // Create GHL opportunities for each supplier
      const { GoHighLevelClient } = await import("./services/gohighlevel-client");
      const ghlClient = new GoHighLevelClient(baseUrl, apiKey, locationId);
      
      const createdOpportunities: Array<{ supplierId: string; supplierName: string; opportunityUrl: string }> = [];
      ...
      for (const [supplierId, supplierItems] of itemsBySupplier) {
        ...
        // Calculate total and build notes
        let subtotal = 0;
        let earliestStockout = Infinity;
        const lineItems: string[] = [];
        
        for (const { item, supplierItem } of supplierItems) {
          const unitCost = supplierItem?.price ?? 10; // Default price if not set
          ...
          stockoutDate.setDate(stockoutDate.getDate() + Math.floor(item.daysOfCover));
          
          lineItems.push(
            `• ${item.sku} | ${item.name}\n` +
            `  Stock: ${item.currentStock}, Stockout: ${stockoutDate.toISOString().split('T')[0]}\n` +
            `  Recommended Qty: ${recommendedQty} @ $${unitCost.toFixed(2)} = $${lineTotal.toFixed(2)}`
          );
        }
        
        // Get lead time for arrival estimate (use max lead time from items)
        const maxLeadTime = Math.max(
          ...supplierItems.map(si => si.supplierItem?.leadTimeDays ?? 7)
        );
        const arrivalDate = new Date();
        arrivalDate.setDate(arrivalDate.getDate() + maxLeadTime);
        
        const opportunityName = `PO Draft – ${supplierName} – ${today}`;
        const notes = `DRAFT PURCHASE ORDER
Supplier: ${supplierName}
Generated: ${today}
Earliest Stockout: ${Math.floor(earliestStockout)} days
Est. Arrival: ${arrivalDate.toISOString().split('T')[0]}
...
⚠️ This is a system-generated draft. Please review before sending to supplier.`;
        ...
        if (result.success && result.opportunityUrl) {
          ...
          // Set primary URL to the most urgent supplier (first in sorted list)
          if (!primaryUrl) {
            primaryUrl = result.opportunityUrl;
          }
        } else {
          console.warn(`[Fix in GHL] Failed to create opportunity for ${supplierName}:`, result.error);
        }
      }
      
      res.json({
        message: createdOpportunities.length > 0 
          ? `Created ${createdOpportunities.length} draft PO opportunities in GoHighLevel`
          : "Failed to create opportunities. Check GHL configuration.",
      ...
      res.status(500).json({ 
        error: error.message || "Failed to create GHL opportunities",
        primaryGhlOpportunityUrl: null
      });
    }
  });
