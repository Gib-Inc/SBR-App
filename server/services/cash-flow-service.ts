...
export function debtTier(name: string, type: string): Tier {
  ...
  if (/shopify capital|fresh funding|paypal|loanbuilder|uncapped|capital on tap|fora|mca/.test(n)) return "mca";
...
export function rankAndProject(obls: Obligation[], cashAvailable: number, asOf: string): Obligation[] {
  ...
  for (const o of ranked) {
    if (o.status === "paid" || o.status === "deferred") { o.runningCashAfter = null; continue; }
...
export async function getCashPosition(db: any, windowDays: number, asOf: string): Promise<CashPosition> {
  const cashRow = rows(await db.execute(sql`
    ...
    from qb_financial_snapshots order by captured_at desc limit 1`))[0];
...
export async function getCashFlow(db: any, opts: { windowDays?: number; asOf?: string } = {}): Promise<CashFlowResult> {
  ...
  const raw = rows(await db.execute(sql`
    ...
    where is_active and status <> 'paid'
      and (due_date is null or due_date <= (current_date + ${windowDays}::int))`));
  ...
  const active = ranked.filter((o) => o.status !== "deferred");
...
export async function setObligationStatus(db: any, id: string, status: OblStatus, by?: string, byName?: string): Promise<void> {
